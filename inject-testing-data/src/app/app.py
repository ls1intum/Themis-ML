...
@app.get("/inject/api/repository/{participation_id}/files-content")
def get_repository(participation_id: int):
    return get_test_submission(participation_id)


@app.get("/inject/api/programming-exercise-participations/{participation_id}/latest-result-with-feedbacks")
...
@app.get("/inject/ids/{exercise_id}")
def get_ids(exercise_id: int):
    return get_participation_ids(exercise_id)


@app.get("/inject/notify_for_exercise/{exercise_id}")
